...
def train_target_depth_seg(model, trainloader, targetloader, cfg):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)):
        ...
        _, pred_src_main, pred_depth_src_main, seg_depth_src = model(images_source.cuda(device))
        seg_depth_src = interp(seg_depth_src)
        ...
        loss_seg_depth_src_main = loss_calc(seg_depth_src, labels, device)
        loss += cfg.TRAIN.LAMBDA_SEG_FUSE_MAIN * loss_seg_depth_src_main
        loss.backward()
        ...
        _, pred_trg_main, pred_depth_trg_main, seg_depth_trg = model(images.cuda(device))
        seg_depth_trg = interp_target(seg_depth_trg)
        ...
        d_out_depth_seg_main = d_main(prob_2_entropy(F.softmax(pred_trg_main))*seg_depth_trg)
        d_out_depth_seg_main = bce_loss(d_out_depth_seg_main, source_label)
        loss += cfg.TRAIN.LAMBDA_ADV_MAIN * d_out_depth_seg_main
        ...
        seg_depth_src = seg_depth_src.detach()
        d_out_main = d_main(prob_2_entropy(F.softmax(pred_src_main)) * pred_depth_src_main)
        loss_d_main = bce_loss(d_out_main, source_label)
        d_out_depth_seg_main = d_main(prob_2_entropy(F.softmax(pred_src_main)) * seg_depth_src)
        loss_d_main = loss_d_main / 2 + bce_loss(d_out_depth_seg_main, source_label) / 2
        loss_d_main.backward()

        # train with target
        pred_trg_main = pred_trg_main.detach()
        pred_depth_trg_main = pred_depth_trg_main.detach()
        seg_depth_trg = seg_depth_trg.detach()
        d_out_main = d_main(prob_2_entropy(F.softmax(pred_trg_main)) * pred_depth_trg_main)
        loss_d_main = bce_loss(d_out_main, target_label)
        d_out_depth_seg_main = d_main(prob_2_entropy(F.softmax(pred_trg_main)) * seg_depth_trg)
        loss_d_out_depth_seg_main = bce_loss(d_out_depth_seg_main, target_label)
        loss_d_main = loss_d_main / 2 + loss_d_out_depth_seg_main / 2
        loss_d_main.backward()
        ...
        current_losses = {
            ...
            "loss_depth_trg_main": loss_depth_trg_main,
            "loss_adv_trg_main": loss_adv_trg_main,
            "loss_d_out_depth_seg_main": loss_d_out_depth_seg_main,
