...
class ResNetMultiDepth(ResNetMulti):
    def __init__(self, block, layers, num_classes, multi_level):
        ...
        self.dec4depth = nn.Conv2d(128, 2048, kernel_size=1, stride=1, padding=0, bias=True)
        self.layer6_depth = ClassifierModule(2048, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
        self.enc4_1.weight.data.normal_(0, 0.01)
        self.enc4_2.weight.data.normal_(0, 0.01)
        self.enc4_3.weight.data.normal_(0, 0.01)
        self.dec4depth.weight.data.normal_(0, 0.01)
    ...
    def forward(self, x):
        ...
        x4_enc = self.relu(x4_enc)
        x4_enc = self.enc4_3(x4_enc) # feather depth
        depth = torch.mean(x4, dim=1, keepdim=True)  # depth output
        # decoder
        x4_dec = self.dec4depth(x4_enc)
        x4_dec = self.relu(x4_dec)
        x4_f_depth = x4 * x4_dec  # feat-fusion
        seg_conv5 = self.layer6(x4)  # produce segmap 2
        seg_depth = self.layer6_depth(x4_f_depth) # seg with depth

        return seg_conv4, seg_conv5, depth, seg_depth
    ...
    def get_10x_lr_params(self):
        ...
        b.append(self.dec4depth.parameters())
        b.append(self.layer6_depth.parameters())
...
def get_deeplab_v2_depth_decoder_separate_branch(num_classes=16, multi_level=False):
