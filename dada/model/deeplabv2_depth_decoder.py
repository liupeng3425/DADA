...
class ResNetMultiDepth(ResNetMulti):
    ...
    def forward(self, x):
        ...
        depth = torch.mean(x4_dec, dim=1, keepdim=True)  # depth output
        x4_dec = self.relu(x4_dec)
        # x4 = x4 * x4_dec  # feat-fusion
        seg_conv5 = self.layer6(x4)  # produce segmap 2
        seg_depth = self.layer6_depth(x4_dec) # seg with depth
...
def get_deeplab_v2_depth_decoder(num_classes=16, multi_level=False):
